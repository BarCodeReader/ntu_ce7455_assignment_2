...
if __name__ == "__main__":
    ...
    if opt.model_type.lower() == "lstm":
        encoder1 = EncoderLSTM(
            input_lang.n_words, opt.hidden_size, device, is_bidirectional=False
        ).to(device)
        decoder1 = DecoderLSTM(
            opt.hidden_size, output_lang.n_words, device, is_bidirectional=False
        ).to(device)
    else:
        encoder1 = EncoderLSTM(
            input_lang.n_words, opt.hidden_size, device, is_bidirectional=True
        ).to(device)
        decoder1 = DecoderLSTM(
            opt.hidden_size, output_lang.n_words, device, is_bidirectional=True
        ).to(device)
